...
//
// 1) NoSQL injection defense — MongoDB has no SQL, but the equivalent
//    risk exists: if a raw client-supplied value (e.g. `req.query.limit`
//    as an object like `{ $ne: null }` via a crafted query string) is
//    ever spread directly into a Mongo filter, it can bypass intended
//    query logic. RULE: every value that reaches a Mongo filter/option
//    MUST go through one of the coercion helpers below first — never
//    pass `req.query`/`req.body` fields through unchanged.
//
...
/**
 * Truncates a string to at most `max` characters. Used for anything that
 * flows into an LLM prompt (chat query, conversation history) — an
 * unbounded string is both a cost/abuse vector (huge input = huge token
 * bill) and, in pathological cases, a way to push real instructions out
